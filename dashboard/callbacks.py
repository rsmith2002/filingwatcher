...
from dashboard.components.backtest_charts import (
    equity_curve_fig, monthly_heatmap_fig, trade_histogram_fig, trade_scatter_fig,
)
...
def _query_flags(tickers: list[str]) -> pd.DataFrame:
    ...
    try:
        ...
        rows = q.order_by(Flag.flagged_at.desc()).limit(2000).all()
        if not rows:
            return pd.DataFrame()
        return pd.DataFrame([{
            c.key: getattr(r, c.key)
...
def register_callbacks(app):
    ...
    @app.callback(
        ...
        Output("bt-chart-heatmap",  "figure"),
        Output("bt-chart-histogram","figure"),
        Output("bt-chart-scatter",  "figure"),
        Output("grid-trades",       "rowData"),
        Input("store-backtest-results", "data"),
        prevent_initial_call=True,
    )
    def update_backtest_display(results_json):
        """Render stat cards and charts from the stored backtest results."""
        import dash_bootstrap_components as dbc
        from dashboard.components.insider_profile import _stat_card
        from dashboard.components.charts import THEME as T
        from dash import html

        empty_figs = (
            equity_curve_fig(pd.Series(dtype=float), pd.Series(dtype=float)),
            monthly_heatmap_fig(pd.Series(dtype=float)),
            trade_histogram_fig(pd.DataFrame()),
            trade_scatter_fig(pd.DataFrame()),
        )
        ...
        if "error" in data:
            err_card = dbc.Alert(data["error"], color="danger",
